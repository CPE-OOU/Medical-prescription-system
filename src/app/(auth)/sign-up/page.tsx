...
type SignupFormData = TypeOf<typeof formSchema>;
  ...
  const signUserIn = form.handleSubmit(async function signUserIn(formData) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        router.push('/');
        toast.success('Account', { description: 'User account created' });
      }

      const responsePayload: FailedServerResponsePayload =
        await response.json();

      switch (response.status) {
        case StatusCodes.CONFLICT:
        case StatusCodes.BAD_REQUEST: {
          return toast.error(responsePayload.title, {
